...
void openURL(UsageEnvironment& env, char const* progName, char const* rtspURL, char const * username, char const* password);
...
void usage(UsageEnvironment& env, char const* progName) {
  env << "Usage: " << progName << " <rtsp-url> username password\n";
...
int main(int argc, char** argv) {
  ...
  openURL(*env, argv[0], argv[1], argv[2], argv[3]);
...
void setupNextSubsession(RTSPClient* rtspClient) {
  ...
  if (scs.session->absStartTime() != NULL) {
    ...
    rtspClient->sendPlayCommand(*scs.session, continueAfterPLAY, 0, -1, 1.0f, ourAuthenticator);
  }
}
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString) {
  do {
    ...
    if (strcmp(scs.subsession->codecName(), "H264") == 0) {
	    scs.subsession->sink = Imx6H264Sink::createNew(env,
						  scs.subsession->fmtp_spropparametersets(),
                          NULL, NULL,
						  IPC_DATA_BLOCK_SIZE - 128);
	}
    //  else if(strcmp(scs.subsession->codecName(), "MPEG4-GENERIC") == 0
	//  ||
	...
	//     = ADTSAudioStreamDiscreteFramer::createNew(env, scs.subsession->readSource(),
	// 					       scs.subsession->fmtp_config());
	//   scs.subsession->addFilter(adtsFramer);

    //   scs.subsession->sink = FileSink::createNew(env, "stdout");
	// } 
    // else {
